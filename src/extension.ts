...
let poemPanel: vscode.WebviewPanel | undefined;
...
export function activate(context: vscode.ExtensionContext) {
  ...
  fs.writeFileSync(
    csvFilePath,
    'unix_timestamp,adversity,suggestion_preference,suggestion_relevance,event_type,suggestion_text,suggestion_rating\n'
  );

  // ---- utility: single place to append CSV rows
  function logEvent(event_type: string, opts?: { suggestion_text?: string; rating?: string; adversity?: string; relevance?: string; pref?: string; ts?: number }) {
    ...
    try { fs.appendFileSync(csvFilePath!, row, 'utf-8'); } catch { /* no-op */ }
  }
  ...
  function showPoemPanel() {
    ...
    poemPanel = vscode.window.createWebviewPanel(
      'poemTask',
      'Task: Copy the Poem',
...
${referenceText}
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('extension.showPoemTask', showPoemPanel)
  );
  showPoemPanel();
  ...
  function showSliderPanel() {
    ...
    sliderPanel.webview.onDidReceiveMessage(
      msg => {
  ...
  setInterval(() => {
    if (!isFeedbackActive) showSliderPanel();
  }, 60000);

  // ---- Ask frequency and (optionally) apply accept
  type ActionKind = 'accept' | 'reject' | 'ignored_no_reveal' | 'reveal_dismissed';

  function getRatingAndExecute(suggestion: string, position: vscode.Position, editor: vscode.TextEditor, action: ActionKind) {
    isFeedbackActive = true;
    ...
    vscode.window.showQuickPick(options, {
      ...
      if (rating) {
        ...
        const dismissedEvent =
          action === 'accept' ? 'accept_clicked_pref_prompt_dismissed'
  ...
  function showSuggestionPopup(suggestion: string, position: vscode.Position, editor: vscode.TextEditor) {
